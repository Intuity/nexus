...
async def memory(dut, index, en_wstrb):
    ...
    u_memory = getattr(dut.u_dut.u_control, f"gen_top_mem[{index}]").u_memory

    # Get path to receiving node
    u_node = dut.u_dut.u_mesh.gen_rows[0].gen_columns[columns-TOP_MEM_COUNT+index].u_node
    ...
    for _ in range(100):
        ...
        dut.ctrl_in.append(StreamTransaction(data=req.pack()))
        ...
        # Queue up a response on reads
            ...
            dut.expected.append(StreamTransaction(data=resp.pack()))
    ...
    while dut.u_dut.u_mesh.i_trigger == 0: await RisingEdge(dut.clk)
    ...
    for msg in last_outs:
        msg.stamp += 1
        dut.expected.append(StreamTransaction(data=msg.pack()))
