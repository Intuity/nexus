...
PYBIND11_MODULE(nxfastmodel, m) {
    ...
    py::class_<NXMessagePipe, std::shared_ptr<NXMessagePipe>>(m, "NXMessagePipe")
        .def(py::init<>())
        .def("enqueue", static_cast<void (NXMessagePipe::*)(node_load_instr_t  )>(&NXMessagePipe::enqueue))
        .def("enqueue", static_cast<void (NXMessagePipe::*)(node_map_output_t  )>(&NXMessagePipe::enqueue))
        .def("enqueue", static_cast<void (NXMessagePipe::*)(node_sig_state_t   )>(&NXMessagePipe::enqueue))
        .def("enqueue", static_cast<void (NXMessagePipe::*)(node_raw_t         )>(&NXMessagePipe::enqueue))
        .def("dequeue", static_cast<void (NXMessagePipe::*)(node_load_instr_t &)>(&NXMessagePipe::dequeue))
        .def("dequeue", static_cast<void (NXMessagePipe::*)(node_map_output_t &)>(&NXMessagePipe::dequeue))
        .def("dequeue", static_cast<void (NXMessagePipe::*)(node_sig_state_t  &)>(&NXMessagePipe::dequeue))
